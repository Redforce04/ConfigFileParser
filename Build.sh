rm ConfigFileParser\VersionInfo.cs
cp ConfigFileParser\BlankVersionInfo.txt ConfigFileParser\VersionInfo.cs
...
dotnet run --project "ReplaceTextWithVariables" ConfigFileParser/VersionInfo.cs -d {BUILD_DATE_TIME} -cmd "echo true" {PORTABLE} -cmd "git describe --tags --abbrev=0" {CI_COMMIT_VERSION_TAG} -cmd "git describe --long --always --exclude=* --abbrev=8" {CI_COMMIT_SHORT_SHA} -cmd "git branch --show-current" {CI_COMMIT_BRANCH}
